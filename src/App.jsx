...
const TOGGLES = [
  ...
  { id:"alc",    label:"お酒を飲まない",        sub:"下戸・禁酒の相手を求める場合はON", value:0.30, adviceLabel:"飲酒OKにすると" },
  { id:"smoke",  label:"タバコを吸わない",      sub:"非喫煙者限定の場合はON",           value:0.76, adviceLabel:"喫煙OKにすると" },
];
...
const INCOME_JOB_CORR = { 0:1.0, 400:1.01, 500:1.01, 600:1.02, 700:1.02, 800:1.03, 1000:1.03 };
const AGE_SINGLE_CORR = { 25:0.984, 27:0.974, 30:0.937, 33:0.885, 36:0.822 };
const AGE_LOC_CORR    = 1.6;
const JOB_LOC_CORR    = 1.15;
const HEIGHT_INCOME_CORR = {
  "0.86_400":1.02,"0.86_500":1.03,"0.86_600":1.04,"0.86_700":1.05,"0.86_800":1.05,"0.86_1000":1.05,
  "0.58_400":1.03,"0.58_500":1.04,"0.58_600":1.05,"0.58_700":1.06,"0.58_800":1.06,"0.58_1000":1.06,
  "0.26_400":1.04,"0.26_500":1.05,"0.26_600":1.06,"0.26_700":1.08,"0.26_800":1.08,"0.15_1000":1.18,
};
...
function buildBreakdown(incomeOpt, eduOpt, heightOpt, ts, myAgeVal, pop) {
  ...
  for (const t of TOGGLES) {
    ...
    if (t.id === "age") {
      ...
      // 正社員率は学歴が高いほど高い→学歴別に補正値を変える
      const JOB_EDU_RATE = { 1.0:0.75, 0.6:0.83, 0.15:0.90, 0.05:0.96 };
      const jobEduRate = JOB_EDU_RATE[eduOpt.value] ?? 0.75;
      running *= jobEduRate * (INCOME_JOB_CORR[incomeOpt.min] ?? 1.0);
    ...
    <div style={{ background:"#F5F7FA", minHeight:"100vh", padding:"24px 16px 60px", fontFamily:"'Hiragino Sans','Noto Sans JP',sans-serif", color:"#1E3A5F" }}>
      <div style={{ maxWidth:480, margin:"0 auto" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:28 }}>
          ...
          <p style={{ fontSize:13, color:"#4A6A8A", lineHeight:1.7, marginBottom:12 }}>条件を入力すると日本全体と比較して、<br />理想の男性の存在率が分かります</p>
          ...
          <div>
            ...
            <div style={{ background:"#EEF2F7", borderRadius:12, padding:"16px 20px" }}>
              ...
              <div style={{ fontSize:11, color:"#6A8AA8", lineHeight:1.9 }}>
                ...
                ・喫煙率・飲酒率：厚生労働省「国民健康・栄養調査」（令和4〜5年）。喫煙率は22〜35歳の年齢別人口で加重平均（20代男性約19.5%・30代男性約27.0%）→非喫煙率約76%<br />
                ...
                ・【補正⑥】正社員×都内在住：都内勤務者の正社員率が高い相関を反映（×1.2）
                ・【補正⑧】正社員×学歴：学歴が高いほど正社員率が高い相関を反映。こだわらない×0.75・大卒×0.83・MARCH×0.90・早慶旧帝×0.96（厚労省「就業構造基本調査」2022年より）<br />
